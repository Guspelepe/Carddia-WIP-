...
function criarJogadorInicial(id) {
  return { id, hp: 4000, deck: [], mao: [], zonaMonstros: [null, null, null], zonaMagias: [null, null, null], cemiterio: [] };
}
...
function comprarCarta(jogadorId, quantidade = 1) {
  ...
  for (let i = 0; i < quantidade; i++) {
    ...
    let carta;
    // Delega o "trapaça" do modo difícil para a IA
...
function montarDeck(jogadorId, deckCompleto, nivel) {
  // Se for a IA, delega a montagem do deck para o ai.js
  if (jogadorId === 2) {
      return window.aiMontarDeck ? window.aiMontarDeck(jogadorId, deckCompleto, nivel) : deckCompleto.slice(0, 20);
  }

  // Lógica de sorteio para o JOGADOR (Você) - Normal e Justo
  let deck = [];
  ...
  // Sorteio equilibrado para o jogador, independente da dificuldade
...
function encerrarTurno() {
  ...
  estado.jogadores[1].zonaMonstros.forEach(m => {
    if (m) { m.jaAtacou = false; m.ataquesRestantes = m.efeito === 'pode_atacar_duas_vezes' ? 2 : 1; m.invocadoEsteTurno = false; m.posicaoMudouEsteTurno = false; }
  });
...
function selecionarCartaDaMao(index) {
  ...
  if (carta.tipo === 'monstro') {
    ...
    if (carta.efeito === 'buff_500' || carta.efeito === 'buff_1000') {
      ...
      destacarMonstrosProprios(); adicionarLog(`Escolha um monstro seu para receber +${estado.acaoPendente.valor} de ATK/DEF.`);
    } else if (carta.efeito === 'destruir_inimigo') {
      estado.acaoPendente = { tipo: 'magia_destruir' }; destacarMonstrosInimigos(); adicionarLog('Escolha um monstro inimigo para destruir.');
    } else if (carta.efeito === 'destruir_todos_inimigos') { usarMagia(1, index, null);
      ...
      estado.acaoPendente = { tipo: 'roubar' }; destacarMonstrosInimigos(); adicionarLog('Escolha um monstro inimigo para assumir o controle.');
...
function invocarMonstro(jogadorId, maoIndex, slotIndex, posicao) {
  ...
  const monstro = { ...carta, posicao, jaAtacou: false, ataquesRestantes: carta.efeito === 'pode_atacar_duas_vezes' ? 2 : 1, bonusAtk: 0, bonusDef: 0, temporario: false, invocadoEsteTurno: true, posicaoMudouEsteTurno: false };
...
function aplicarEfeitoInvocacao(jogadorId, slotIndex) {
  ...
  switch (monstro.efeito) {
    case 'ao_invocar_revive_monstro':
      ...
      break;
    case 'ao_invocar_destroi_magia_armadilha': for (let i = 0; i < oponente.zonaMagias.length; i++) { if (oponente.zonaMagias[i]) { adicionarLog(`${monstro.nome} destruiu ${oponente.zonaMagias[i].nome}.`); oponente.zonaMagias[i] = null; } } render(); break;
...
function verificarFimDeDuelo() {
  ...
  if (estado.jogadores[1].hp <= 0) { finalizarDuelo(2, 'HP zerado'); return true; }
...
function destacarAtacantesDisponiveis(jogadorId) {
  ...
  slots.forEach(slot => {
    ...
    if (monstro && monstro.posicao === 'ataque' && monstro.ataquesRestantes > 0 && monstro.efeito !== 'nao_pode_atacar') slot.classList.add('highlight');
  });
}
...
function selecionarAtacante(slotIndex) {
  ...
  estado.atacanteSelecionado = slotIndex; limparDestaques(); destacarAlvosInimigos();
...
async function executarAtaque(atacanteSlot, alvoTipo, alvoSlot) {
  ...
  if (alvoTipo === 'jogador' && temMonstrosInimigos) { adicionarLog('Você não pode atacar diretamente enquanto houver monstros inimigos.'); return; }
  ...
  if (alvoTipo === 'jogador') {
    defensor.hp -= atacante.atk; adicionarLog(`${atacante.nome} atacou diretamente! Computador perdeu ${atacante.atk} HP.`); atacante.ataquesRestantes--; if (verificarFimDeDuelo()) return;
  } else {
    if (monstroDefensor) { resolverBatalha(1, 2, atacanteSlot, alvoSlot); atacante.ataquesRestantes--; }
  }
  estado.processandoAnimacao = false;
...
function resolverBatalha(jogadorAtacanteId, jogadorDefensorId, slotAtacante, slotDefensor) {
  ...
  if (defensor.posicao === 'ataque') {
    ...
    if (dif > 0) { destruirMonstro(jogadorDefensorId, slotDefensor, 'batalha'); estado.jogadores[jogadorDefensorId].hp -= dif; adicionarLog(`${defensor.nome} destruído! Computador perdeu ${dif} HP.`); if (atacante.efeito === 'ganha_500_atk_ao_destruir_inimigo') { atacante.bonusAtk = (atacante.bonusAtk || 0) + 500; adicionarLog(`${atacante.nome} ganhou +500 de ATK.`); } if (atacante.efeito === 'cura_vida_igual_dano_causado') { estado.jogadores[jogadorAtacanteId].hp += dif; adicionarLog(`${atacante.nome} curou ${dif} de vida.`); } verificarFimDeDuelo(); } 
    else if (dif === 0) { destruirMonstro(jogadorAtacanteId, slotAtacante, 'batalha'); destruirMonstro(jogadorDefensorId, slotDefensor, 'batalha'); adicionarLog(`Empate! ${atacante.nome} e ${defensor.nome} destruídos.`); verificarFimDeDuelo(); } 
    ...
    const dif = atkTotal - defTotal;
    if (dif > 0) { destruirMonstro(jogadorDefensorId, slotDefensor, 'batalha'); adicionarLog(`${defensor.nome} destruído em defesa.`); if (atacante.efeito === 'dano_perfurante' || atacante.efeito === 'ignora_defesa_ataque_direto') { estado.jogadores[jogadorDefensorId].hp -= dif; adicionarLog(`Dano perfurante! Computador perdeu ${dif} HP.`); } verificarFimDeDuelo(); } 
...
function renderZonas() {
  for (let i = 1; i <= 2; i++) {
    const zonaMonstros = document.getElementById(`monstro-slots-p${i}`); const zonaMagias = document.getElementById(`magia-slots-p${i}`);
    ...
    for (let j = 0; j < 3; j++) {
      ...
      if (monstro) {
        ...
        const atkTotal = monstro.atk + (monstro.bonusAtk || 0); const defTotal = monstro.def + (monstro.bonusDef || 0);
...
function renderLog() { const logDiv = document.getElementById('log'); logDiv.innerHTML = estado.log.slice(-10).map(msg => `<div>${msg}</div>`).join(''); logDiv.scrollTop = logDiv.scrollHeight; }
...
function handleSlotClick(jogadorId, zona, slotIndex) {
  ...
  else if (acao.tipo === 'magia_buff') { if (jogadorId === 1 && zona === 'monstro' && jogador.zonaMonstros[slotIndex] !== null) { usarMagia(1, estado.cartaSelecionada, { tipo: 'proprio', slot: slotIndex }); } } 
  else if (acao.tipo === 'magia_destruir') { if (jogadorId === 2 && zona === 'monstro' && estado.jogadores[2].zonaMonstros[slotIndex] !== null) { usarMagia(1, estado.cartaSelecionada, { tipo: 'inimigo', slot: slotIndex }); } } 
  else if (acao.tipo === 'roubar') { if (jogadorId === 2 && zona === 'monstro' && estado.jogadores[2].zonaMonstros[slotIndex] !== null) { usarMagia(1, estado.cartaSelecionada, { tipo: 'inimigo', slot: slotIndex }); } }
}
